...
class RSIStrategy(BaseStrategy):
    ...
    CONFIG_SCHEMA = {
        ...
        'timeframe': {'label': '타임프레임', 'type': 'select', 'options': ['15m', '30m', '1h', '4h'], 'default': '1h'},
        'overbought': {'label': '과매수 기준', 'type': 'number', 'min': 60, 'max': 90, 'step': 1, 'default': 70},
        'oversold': {'label': '과매도 기준', 'type': 'number', 'min': 10, 'max': 40, 'step': 1, 'default': 30},
        'rsi_period': {'label': 'RSI 기간', 'type': 'number', 'min': 7, 'max': 28, 'step': 1, 'default': 14},
        'use_trend_filter': {'label': 'EMA 트렌드 필터', 'type': 'checkbox', 'default': True},
    ...
    def get_indicator_display_schema(self) -> List[Dict]:
        """RSI 전략 지표 표시 스키마 (AdaptiveExitManager 기본 지표 사용)"""
        return [
