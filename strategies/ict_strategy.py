...
class ICTStrategy(BaseStrategy):
    ...
    CONFIG_SCHEMA = {
        ...
        'use_ai': {'label': '🤖 AI 검증', 'type': 'checkbox', 'default': True},
        'timeframe': {'label': '타임프레임', 'type': 'select', 'options': ['5m', '15m', '30m', '1h'], 'default': '15m'},
        'ob_lookback': {'label': 'OB 탐지 범위', 'type': 'number', 'min': 5, 'max': 50, 'step': 1, 'default': 20},
        'fvg_min_gap_pct': {'label': 'FVG 최소 갭 (%)', 'type': 'number', 'min': 0.0001, 'max': 0.01, 'step': 0.0001, 'default': 0.001},
        'min_confidence': {'label': '최소 신뢰도', 'type': 'number', 'min': 0, 'max': 1, 'step': 0.05, 'default': 0.65},
        'min_risk_reward': {'label': '최소 R/R', 'type': 'number', 'min': 0.5, 'max': 5, 'step': 0.1, 'default': 1.5},
        'atr_multiplier': {'label': 'ATR 배수', 'type': 'number', 'min': 0.5, 'max': 5, 'step': 0.1, 'default': 2.0},
        'proximity_pct': {'label': '근접 비율 (%)', 'type': 'number', 'min': 0.001, 'max': 0.05, 'step': 0.001, 'default': 0.01},
        'use_volume_filter': {'label': '볼륨 필터', 'type': 'checkbox', 'default': True},
    ...
    def get_indicator_display_schema(self) -> List[Dict]:
        ...
        return [
            ...
            {"key": "ema20", "label": "EMA20", "align": "right", "format": "price"},
    ...
    def compute_realtime_indicators(self, candles: List[Dict]) -> Optional[Dict]:
        """ICT 지표 실시간 계산 (Keltner, volume_ratio 등)"""
        if len(candles) < 50:
            return None
        try:
            ...
            last = df.iloc[-1]
            out = {}
            for k in ['ema20', 'keltner_upper', 'keltner_lower', 'rsi', 'atr']:
