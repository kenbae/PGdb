...
def main():
    """메인 함수"""
    print("🚀 watched_symbols 테이블 생성 시작...")
    ...
    # SQL 파일 읽기
    sql_file = Path(__file__).parent / "database" / "watched_symbols_schema.sql"
    ...
    try:
        ...
        print("✅ watched_symbols 테이블 생성 완료!")

        # 확인
        with engine.connect() as conn:
            result = conn.execute(text("SELECT * FROM watched_symbols ORDER BY created_at"))
            rows = result.fetchall()

            print(f"\n📋 현재 감시 심볼: {len(rows)}개")
            for row in rows:
                print(f"  - {row[1]} ({row[2]}) [enabled={row[3]}]")
