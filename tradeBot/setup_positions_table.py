...
def main():
    ...
    # SQL 파일 읽기
    sql_file = Path(__file__).parent / "database" / "positions_schema.sql"
    ...
    try:
        ...
        print("✅ 포지션 히스토리 테이블 생성 완료!")

        # 확인
        with engine.connect() as conn:
            # tradebot_positions 테이블
            result = conn.execute(text("SELECT COUNT(*) FROM tradebot_positions"))
            count = result.fetchone()[0]
            print(f"\n📋 tradebot_positions 테이블: {count}개 레코드")
